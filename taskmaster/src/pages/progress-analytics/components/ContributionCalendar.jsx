...
const ContributionCalendar = ({ data }) => {
  ...
  const getMonthLabels = () => {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      // Only push label if it's different from the last one or sparsely distributed
      // For simplicity, we just push all 12 stats for the full year view
  ...
  return (
    <div className="bg-card/50 backdrop-blur-sm border border-border/50 rounded-xl p-6 shadow-sm">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-xl font-semibold text-foreground tracking-tight">Active Days</h3>
          <p className="text-sm text-muted-foreground mt-1">
            {data?.filter(d => d?.completion > 0)?.length} days with activity this year
          </p>
        </div>
      ...
      <div className="overflow-x-auto pb-4 scrollbar-hide">
        <div className="inline-flex flex-col gap-1 min-w-full">
          {/* Note: This grid assumes 'data' is a flat array of 365 days sorted by date. 
                If 'data' is just fetch results, we might need a processing step to fill gaps. 
                Assuming 'data' is correctly processed in parent for now. */}
            ...
            {data?.map((day) => (
              ...
              >
                <div
                  ...
                  style={getIntensityStyle(day?.completion)}
                />
                ...
                <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 z-50 hidden group-hover:block pointer-events-none">
                  <div className="bg-popover/95 backdrop-blur-md border border-border rounded-lg p-3 shadow-xl whitespace-nowrap min-w-[140px]">
                    ...
                    <div className="flex items-center gap-2 mb-1">
                      <span className="w-1.5 h-1.5 rounded-full" style={getIntensityStyle(day?.completion)}></span>
